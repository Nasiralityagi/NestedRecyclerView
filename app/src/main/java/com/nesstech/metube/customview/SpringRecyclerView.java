...
public class SpringRecyclerView extends TempRecyclerView {
    ...
    private static final int DEF_RELEASE_BACK_ANIM_DURATION = 200; // ms
    private static final int DEF_FLING_BACK_ANIM_DURATION = 60;
    ...
    private float mSpringStretchingLimit = 0.03f;
    ...
    public void draw(Canvas canvas) {
        if (mState == STATE_NORMAL) {
            ...
            if (mOrientation == VERTICAL) {
                ...
                final float scaleY = 1 + Math.abs(mOffset) / viewHeight * mSpringStretchingLimit;
                canvas.scale(1, scaleY, 0, mOffset >= 0 ? 0 : (viewHeight + getScrollY()));
            } else {
                final int viewWidth = getWidth();
                final float scaleX = 1 + Math.abs(mOffset) / viewWidth * mSpringStretchingLimit;
    ...
    public void setSpringStretchingLimit(float limit) {
        mSpringStretchingLimit = limit;
    }
